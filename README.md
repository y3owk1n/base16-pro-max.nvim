...
| **YAML & JSON schema loading**         | Load any Base16 YAML or JSON scheme directly — no manual color copying required.                             |
...
### Using Base16 YAML/JSON schemes

```lua
-- Load from a Base16 scheme file (YAML or JSON)
{
  "y3owk1n/base16-pro-max.nvim",
  priority = 1000,
  config = function()
    ...
    require("base16-pro-max").setup {
      ...
      colors = parser.get_base16_colors("~/.config/nvim/schemes/gruvbox-dark.yaml"),
      styles = { italic = true, transparency = true },
      plugins = { enable_all = true },
    }
    vim.cmd.colorscheme "base16-pro-max"
...
Or with a JSON palette (e.g. from Stylix):
...
```lua
{
  ...
  config = function()
    ...
    require("base16-pro-max").setup {
      -- Load colors from Stylix palette.json
      colors = parser.get_base16_colors("~/.config/stylix/palette.json"),
      styles = { italic = true, transparency = true },
      plugins = { enable_all = true },
    }
    vim.cmd.colorscheme "base16-pro-max"
...
## 📁 YAML & JSON Scheme Support

The plugin includes a high-performance YAML and JSON parser specifically designed for Base16 schemes.
...
local colors = parser.get_base16_colors("~/path/to/scheme.yaml") -- or `yml` extension

-- Or load from a JSON file (e.g. Stylix palette.json)
...
Your YAML and JSON files should follow the standard Base16 format:
...
`palette` nesting (as generated by [Stylix](https://github.com/danth/stylix)) is also supported:
...
-- Option 2: Load from a JSON file (e.g. Stylix palette.json)
local colors = parser.get_base16_colors("~/.config/stylix/palette.json")

-- Option 3: Mix parsed colors with manual overrides
...
-- Parser API (.yaml, .yml, .json)
local colors = parser.get_base16_colors("path/to/scheme.yaml")
local colors = parser.get_base16_colors("path/to/palette.json")
parser.clear_cache()
local stats = parser.get_cache_stats()
```
...
**YAML/JSON loading issues:**

- Verify the file exists and is readable
- Ensure the file follows Base16 format with base00-base0F keys
- JSON files should be valid JSON (no trailing commas)
