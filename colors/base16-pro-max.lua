...
local ok, base16_pro_max = pcall(require, "base16-pro-max")
if not ok then
  vim.notify("base16-pro-max.nvim not installed", vim.log.levels.ERROR)
  return
end
...
if not base16_pro_max.config or not next(base16_pro_max.config) then
  base16_pro_max.setup({}) -- user must have filled M.config.colors elsewhere
end

base16_pro_max.colorscheme() -- apply highlights

-- Tell Neovim which colorscheme is active
vim.g.colors_name = "base16-pro-max"
